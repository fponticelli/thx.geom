...
class Svg {
  public static function parsePath(d : String) : Array<Segment<Dynamic>> {
    ...
    function capturePoints(qt : Int) {
      var c = capture(qt * 2);
      return c.splitBy(2).map(function(p) return Point.create(p[0], p[1]));
    }
    ...
    while(d.length > 0) {
      ...
      switch c {
        ...
        case "L":
          ...
          smooth = null;
          var p = capturePoint();
          list.push(new LineSegment(last, p));
          last = p;
        case "l":
          prev = "l";
          smooth = null;
        ...
        case "Q":
          ...
          list.push(new QuadraticCurveSegment(last, ps[0], ps[1]));
          smooth = ps[0];
          last = ps[1];
        case "q":
          prev = "q";
          var ps = capturePoints(2);
          list.push(new QuadraticCurveSegment(last, last + ps[0], last + ps[1]));
          smooth = last + ps[0];
          last = last + ps[1];
        case "C":
          prev = "C";
          var ps = capturePoints(3);
          list.push(new CubicCurveSegment(last, ps[0], ps[1], ps[2]));
          smooth = ps[1];
          last = ps[2];
        case "c":
          prev = "c";
          var ps = capturePoints(3);
          list.push(new CubicCurveSegment(last, last + ps[0], last + ps[1], last + ps[2]));
          smooth = last + ps[1];
          last = last + ps[2];
        case "H":
          prev = "H";
        ...
        case "S":
          ...
          list.push(new CubicCurveSegment(last, smooth, ps[0], ps[1]));
          smooth = ps[0];
          last = ps[1];
        case "s":
          smooth = null;
          ...
          list.push(new CubicCurveSegment(last, smooth, last + ps[0], last + ps[1]));
          smooth = last + ps[0];
          last = last + ps[1];
        case "T":
          smooth = null;
          ...
          list.push(new QuadraticCurveSegment(last, smooth, p));
          last = p;
        case "t":
          smooth = null;
          prev = "t";
